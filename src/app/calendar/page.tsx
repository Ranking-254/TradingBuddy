...
export default function CalendarPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 pt-2">
      {/* Top Header & Navigation Bar */}
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-3xl bg-[#0e101a] border border-[#1b1d2b]">
        <div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2 bg-[#141624] border border-[#232740] p-1.5 rounded-2xl">
            <button
              onClick={handleToday}
              className="px-3 py-1.5 rounded-xl text-xs font-semibold text-zinc-300 hover:text-white hover:bg-white/5 transition-colors"
            >
              Today
            </button>
            <div className="h-4 w-[1px] bg-[#232740]" />
            <button
      ...
      <div className="bg-[#0e101a] border border-[#1b1d2b] rounded-3xl p-5 shadow-2xl space-y-4">
        {/* Days Header */}
        <div className="grid grid-cols-7 gap-2 text-center text-[11px] font-bold text-zinc-400 uppercase tracking-wider pb-2 border-b border-[#1b1d2b]">
          {DAYS_OF_WEEK.map((d, i) => (
            <div
              key={d}
              className={i === 0 || i === 6 ? "text-zinc-600" : "text-zinc-400"}
            >
              {d}
            </div>
          ))}
          ...
          {Array.from({ length: firstDayIndex }).map((_, idx) => {
            ...
            return (
              <div
                key={`prev-${idx}`}
                onClick={handlePrevMonth}
                className="min-h-[110px] p-2.5 rounded-2xl bg-[#090a10]/40 border border-[#161824]/40 opacity-40 hover:opacity-75 cursor-pointer transition-opacity flex flex-col justify-between"
          ...
          {Array.from({ length: daysInMonth }).map((_, idx) => {
            ...
            return (
              <div
                ...
                className={`min-h-[110px] p-2.5 rounded-2xl border transition-all flex flex-col justify-between relative group ${
                  isTraded
                    ? dayPnL >= 0
                ...
                <div className="flex items-center justify-between">
                  <span
                    className={`text-xs font-mono font-bold ${
                      isToday
                        ? "text-purple-400 bg-purple-500/20 px-1.5 py-0.5 rounded-md"
                        : "text-zinc-300"
                    }`}
                  >
                    {dayNum}
                ...
                {showEvents && dayEvents.length > 0 && (
                  <div className="mt-1 space-y-1">
                    {dayEvents.slice(0, 2).map((ev, i) => (
                      <div
                        key={i}
                        className="text-[9px] px-1.5 py-0.5 rounded font-bold border truncate text-rose-300 bg-rose-500/15 border-rose-500/30 flex items-center gap-1"
                        title={`${ev.country}: ${ev.title}`}
                      >
                        <Radio className="h-2.5 w-2.5 shrink-0 text-rose-400 animate-pulse" />
                    ...
                    {dayEvents.length > 2 && (
                      <span className="text-[9px] text-zinc-500 font-mono pl-0.5">
                        +{dayEvents.length - 2} more news
                      </span>
                    )}
                  </div>
                )}
                ...
                {isTraded ? (
                  <div className="mt-auto pt-1 space-y-0.5">
                    ...
                    <div className="flex items-center justify-between text-[10px] font-mono text-zinc-400">
                      ...
                      <span className="text-zinc-500">{dayTrades.length}t</span>
                    </div>
                  </div>
                ) : isWeekend ? (
                  <span className="text-[10px] text-zinc-600 font-medium italic mt-auto">
                    Closed
                  </span>
                ) : null}
              </div>
      ...
      {selectedDayModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#0e101a] border border-[#1f2235] w-full max-w-2xl rounded-3xl p-6 md:p-8 shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            {/* Header */}
            ...
            {selectedDayModal.trades.length > 0 && (
              <div className="p-4 rounded-2xl bg-[#141624] border border-[#232740] flex items-center justify-between">
                <div>
                ...
                <div className="text-right">
                  <span className="text-[11px] text-zinc-400">Executions</span>
                ...
                <div className="space-y-2">
                  {selectedDayModal.events.map((ev, i) => (
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="px-1.5 py-0.5 rounded font-black text-[10px] bg-rose-500/20 text-rose-300">
                            {ev.country}
                          </span>
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                  {selectedDayModal.trades.map((trade) => {
                    ...
                    return (
                      <div
                        key={trade.id}
                        className="p-3.5 rounded-xl bg-[#121422] border border-[#1f2238] flex items-center justify-between text-xs"
                      >
                          ...
                          <div>
                            {trade.followedRules ? (
                              <span className="text-emerald-400 text-[10px] inline-flex items-center gap-1">
                                <CheckCircle2 className="h-3 w-3" /> Disciplined
                              </span>
                            ) : (
                              <span className="text-amber-400 text-[10px] inline-flex items-center gap-1 font-bold">
                                <AlertTriangle className="h-3 w-3" /> Rule
                                Breach
                              </span>
                            )}
                          </div>
